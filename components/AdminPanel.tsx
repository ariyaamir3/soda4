            ...
            <div className="flex-1 p-6 overflow-y-auto bg-[#0f0f0f]">
                    ...
                    <form onSubmit={handleSubmit} className="space-y-8">
                        {activeTab === 'general' && (
                            <div className="space-y-4">
                                ...
                                <div className="p-4 bg-purple-900/10 border border-purple-500/20 rounded-lg"><label className="flex items-center gap-2 cursor-pointer"><input type="checkbox" checked={formData.enableDarkRoom ?? false} onChange={(e) => setFormData({...formData, enableDarkRoom: e.target.checked})} className="w-4 h-4 rounded border-white/30 bg-black checked:bg-purple-500" /><span className="text-sm text-white font-bold">فعال‌سازی اتاق تاریک</span></label></div>
                ...
                {activeTab === 'darkroom' && (
                    <div className="flex flex-col h-full">
                        <div className="bg-purple-900/10 border border-purple-500/20 rounded-lg p-4 mb-4 flex items-center justify-between"><div className="flex items-center gap-4"><div className="text-2xl">🕶️</div><div><h3 className="font-bold text-white">اتاق تاریک</h3><p className="text-xs text-white/50">جناب کیارستمی</p></div></div><div className="flex bg-black/50 rounded p-1 text-[10px] font-bold"><button onClick={() => setAiMode('auto')} className={`px-3 py-1 rounded ${aiMode === 'auto' ? 'bg-purple-600' : 'text-white/50'}`}>هوشمند</button><button onClick={() => setAiMode('manual')} className={`px-3 py-1 rounded ${aiMode === 'manual' ? 'bg-yellow-600' : 'text-white/50'}`}>دستی</button></div></div>
                        <div className="flex-1 bg-black/20 border border-white/5 rounded-lg mb-4 p-4 overflow-y-auto space-y-4 min-h-[300px] max-h-[400px]">{chatHistory.map((msg, idx) => (<div key={idx} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}><div className={`max-w-[80%] p-3 rounded-lg text-sm ${msg.role === 'user' ? 'bg-white/10 text-white' : 'bg-purple-900/20 text-purple-100 border border-purple-500/20'}`}>{msg.text}</div></div>))}{isAiThinking && <div className="text-purple-400 text-xs flex items-center gap-2"><Loader2 size={12} className="animate-spin" /> ...</div>}<div ref={chatEndRef}></div></div>
